...
class GaussianWarp(nn.Module):
    def __init__(self, K, node_shape, max_densify_num, sparsification, max_contribution, min_contribution, 
        ...
        super().__init__()
        self.K = K
        self.node_shape = node_shape  # the three dimensions of the uniform 3D grid used for initialization
        self.node_initial_num = np.prod(node_shape)   # total number of Gaussian primitives
        self.max_densify_num = max_densify_num  # upper bound on the number of Gaussian primitives after densification
        self.sparsification = sparsification    # sparsification factor w.r.t. the image grid
        ...
        self.node_position = nn.Parameter(node_coords)                   # initial centre positions of the Gaussian primitives
        self.translation = nn.Parameter(torch.zeros(self.node_initial_num, 3))   # initial translations of the Gaussian primitives

        if self.anisotropy.quaternion:  # if use anisotropic Gaussian primitives
        ...
        if self.anisotropy.scaling:
            ...
            self._node_radius = nn.Parameter(torch.ones(self.node_initial_num, 1) * torch.log(node_radius))  # initial radius of the Gaussian primitives

        if with_quaternion:
            self.quaternion = nn.Parameter(torch.concat([torch.ones(self.node_initial_num, 1), 
                                                         torch.zeros(self.node_initial_num, 3)], dim=1))

        # accumulators for the gradient statistics
    ...
    def reinitialize(self, mask: torch.Tensor):
        self.node_shape = (torch.tensor(mask.shape) / self.sparsification - 1).int().tolist()
        node_position, node_radius = self.make_coors(self.node_shape)
        node_position = node_position.unsqueeze(0).unsqueeze(0).unsqueeze(0)
        ...
        logger.info('Initialized %d Gaussian primitives.' % self.node_position.shape[0])
    ...
    def forward(self, x: torch.Tensor):
        # look up the K nearest Gaussian primitives of every sampled voxel position
    ...
    def densify_and_prune(self, optimizer=None, tag=''):
        ...
        if self.node_position.shape[0] + densified_pts_mask.sum() - pruned_pts_mask.sum() > self.max_densify_num: 
            max_contribution = torch.inf  # skip densification
        ...
        num_densified = int(densified_pts_mask.sum())
        num_pruned = int(pruned_pts_mask.sum())
        ...
        if densified_pts_mask.sum() > 0:
            ...
            if self.anisotropy.scaling:
                ...
                stds = self.node_radius[densified_pts_mask].repeat(1,3)  # standard deviation of the Gaussian primitives
            ...
            if self.with_quaternion:
                ...
                # translation and quaternion parameters
            
            for group in optimizer.param_groups:
                stored_state = optimizer.state.get(group['params'][0], None)  # node_position, radius, translation or quaternion
                extension_tensor = new_param_list[group['name']]   # new_node_position, radius, translation or quaternion
                if stored_state is not None:    
                    stored_state["exp_avg"] = torch.cat((stored_state["exp_avg"], torch.zeros_like(extension_tensor)), dim=0)  # append the state of the new nodes
                    stored_state["exp_avg_sq"] = torch.cat((stored_state["exp_avg_sq"], torch.zeros_like(extension_tensor)), dim=0)  # append the state of the new nodes
                    del optimizer.state[group['params'][0]]   # drop the state of the old parameters
                    group["params"][0] = nn.Parameter(torch.cat((group["params"][0], extension_tensor), dim=0).requires_grad_(True))  # append the parameters of the new nodes
                    optimizer.state[group['params'][0]] = stored_state  # overwrite the optimizer state
                    setattr(self, group['name'], group["params"][0])    # rebind the parameter as an attribute
                else:  # without a stored state only group["params"] has to be updated
                    group["params"][0] = nn.Parameter(torch.cat((group["params"][0], extension_tensor), dim=0).requires_grad_(True))
                    setattr(self, group['name'], group["params"][0])  # rebind the parameter as an attribute
        
        # Prune
        if pruned_pts_mask.sum() > 0:  # True marks the nodes to be pruned
            if pruned_pts_mask.shape[0] < self.node_position.shape[0]:  # the densification above may have added nodes
            ...
            pruned_pts_mask = ~pruned_pts_mask # True marks the nodes to be kept

            for group in optimizer.param_groups:
                stored_state = optimizer.state.get(group['params'][0], None)  # state of the parameters in this group
                if stored_state is not None:
                    ...
                    optimizer.state[group['params'][0]] = stored_state  # overwrite with the new state
                    setattr(self, group['name'], group["params"][0])  # rebind the parameter as an attribute
                else:
                    group["params"][0] = nn.Parameter(group["params"][0][pruned_pts_mask].requires_grad_(True))
                    setattr(self, group['name'], group["params"][0])  # rebind the parameter as an attribute

        self.densification_postfix()  # reset the gradient statistics
        logger.info('%sdensified %d and pruned %d Gaussian primitives, %d left.'
                    % (tag + ' ' if tag else '', num_densified, num_pruned,
                       self.node_position.shape[0]))

    def add_densification_stats(self):  # accumulate gradient
        '''Accumulate the gradient magnitude of the translations.'''
        self.contribution += torch.norm(self.translation.grad, dim=-1)  # accumulate the gradient norm over the xyz dimensions
        self.counter += 1
